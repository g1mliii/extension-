...
// Make components globally available for other modules with error handling
try {
    ...
    window.trustScoreTooltip = trustScoreTooltip;
    window.compactRatingManager = compactRatingManager;
    window.affiliateManager = affiliateManager;
} catch (error) {
    errorHandler.handleComponentError('global-exports', error, { context: 'Making components globally available' });
}
...
function showMessage(text, type = 'info', options = {}) {
    try {
        ...
        // Use enhanced notification manager with error handling
        ...
        errorHandler.handleComponentError('notification-display', error, {
            text, type, options,
...
function updateStatsDisplay(data) {
    try {
        ...
        errorHandler.handleComponentError('stats-display', error, {
            data,
            context: 'Updating stats display'
        });
        ...
        try {
            ...
            if (trustScoreSpan) {
                trustScoreSpan.textContent = '50%';
            }
        } catch (fallbackError) {
            console.error('Stats display fallback failed:', fallbackError);
        }
    }
}
