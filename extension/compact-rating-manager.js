...
class CompactRatingManager {
    ...
    createImpactPreview() {
        ...
        content.className = 'impact-preview-content';
        ...
        value.id = 'impact-value';
        value.textContent = '+0';

        const arrow = document.createElement('span');
        arrow.className = 'impact-arrow';
        arrow.id = 'impact-arrow';
        arrow.textContent = '→';
        ...
        details.id = 'impact-details';

        const beforeAfter = document.createElement('div');
        beforeAfter.className = 'before-after';

        const beforeScore = document.createElement('span');
        beforeScore.className = 'before-score';
        beforeScore.id = 'before-score';
        beforeScore.textContent = '50%';

        const detailsArrow = document.createElement('span');
        detailsArrow.className = 'arrow';
        detailsArrow.textContent = '→';

        const afterScore = document.createElement('span');
        afterScore.className = 'after-score';
        afterScore.id = 'after-score';
        afterScore.textContent = '50%';

        beforeAfter.appendChild(beforeScore);
        beforeAfter.appendChild(detailsArrow);
        beforeAfter.appendChild(afterScore);
    ...
    createRatingGuideTooltip() {
        ...
        header.className = 'rating-guide-header';

        const title = document.createElement('h3');
        title.id = 'rating-guide-title';
        title.textContent = 'Rating Guide';

        const closeBtn = document.createElement('button');
        closeBtn.className = 'rating-guide-close';
        ...
        // Content
        const content = document.createElement('div');
        content.className = 'rating-guide-content';

        // How Ratings Work Section
        const ratingsTitle = document.createElement('h4');
        ratingsTitle.textContent = 'How Ratings Work';
        content.appendChild(ratingsTitle);

        const ratingsList = document.createElement('ul');

        const ratingsItems = [
            { label: '⭐ Star Ratings:', text: "Higher stars improve the site's trust score" },
            { label: '🔴 Quality Issues:', text: 'Flagging issues reduces the trust score' },
            { label: '📊 Community Impact:', text: 'Your rating helps other users make informed decisions' }
        ];
        ...
        // Quality Issue Types Section
        const issuesTitle = document.createElement('h4');
        issuesTitle.textContent = 'Quality Issue Types';
        content.appendChild(issuesTitle);

        const issuesList = document.createElement('ul');

        const issuesItems = [
            { label: 'Spam/Ads:', text: 'Excessive advertising, spam content, or poor functionality' },
            { label: 'Misleading:', text: 'False or deceptive information' },
            { label: 'Suspicious:', text: 'Potentially harmful or fraudulent activity' }
        ];
        ...
        this.ratingGuideTooltip.appendChild(header);
        this.ratingGuideTooltip.appendChild(content);
